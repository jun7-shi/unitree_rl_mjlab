...
G1_UPPER_BODY_JOINT_COLUMNS = tuple(G1_29DOF_JOINT_COLUMNS[12:])
...
def load_seed_upper_body_joint_angles(path: str | Path) -> np.ndarray:
  input_path = Path(path)
  with input_path.open(newline="", encoding="utf-8") as handle:
    reader = csv.DictReader(handle)
    if reader.fieldnames is None:
      raise ValueError(f"Seed G1 CSV has no header: {input_path}")
    missing = [column for column in G1_UPPER_BODY_JOINT_COLUMNS if column not in reader.fieldnames]
    if missing:
      raise ValueError(f"Seed G1 CSV is missing upper-body columns: {', '.join(missing)}")
    rows = [
      [math.radians(float(row[column])) for column in G1_UPPER_BODY_JOINT_COLUMNS]
      for row in reader
    ]
...
def compare_retarget_with_seed_csv(config: CompareConfig) -> CompareSummary:
  ...
  seed_q = load_seed_upper_body_joint_angles(config.seed_csv_path)[frame_slice]
  ...
  per_joint_max = {
    ...
    for column, value in zip(G1_UPPER_BODY_JOINT_COLUMNS, np.max(abs_diff_deg, axis=0))
  }
  return CompareSummary(
...
def build_arg_parser() -> argparse.ArgumentParser:
  parser = argparse.ArgumentParser(
    description="Compare SEW-Mimic BVH retargeting against a bones-seed G1 CSV."
  )
  ...
  parser.add_argument("--top-joints", type=int, default=8, help="Number of largest per-joint differences to print.")
  parser.add_argument(
    "--raw-orientations",
    action="store_true",
